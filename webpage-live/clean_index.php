...
if (isset($_GET['action'])) {
    ...
    try {
        ...
        if ($_GET['action'] === 'get_locations' && isset($_GET['key'])) {
            $stmt = $pdo->prepare("SELECT latitude, longitude, time, velocity, distance, motion_state, time_spent_here FROM clean_location_data WHERE hashed_key = ? ORDER BY time ASC");
...
<head>
    ...
    <title>Clean Tag Tracker Viewer</title>
...
<?php if (empty($_SESSION['authenticated'])): ?>
    <div class="login-wrapper">
        <h2>Cleaned Tracker Dashboard</h2>
        <form method="POST">
    ...
    <script>
        ...
        function drawLocations(locations) {
            ...
            for (let i = 0; i < locations.length; i++) {
                ...
                const timeSpent = loc.time_spent_here !== null ? parseFloat(loc.time_spent_here).toFixed(2) + ' min' : 'N/A';
                ...
                const invisibleCircle = L.circleMarker(currentPoint, {
                    ...
                    weight: 0,
                    fillOpacity: 0
                }).bindPopup(`Time: ${loc.time}<br>State: <b>${state}</b><br>Velocity: ${velocity}<br>Distance: ${distance}<br>Time Spent: ${timeSpent}`).addTo(map);
